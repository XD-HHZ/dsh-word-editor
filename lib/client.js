	...
	factory: (require) => {
		...
		function formatSignature(runs) {
			...
			for (const run of runs || []) {
				const flag = (run.fmt.b ? "b" : "") + (run.fmt.i ? "i" : "") + (run.fmt.u ? "u" : "");
						...
						withBaseline(
							{
								...
								origRuns: runs.map((run) => ({
									text: run.text,
									fmt: { b: run.fmt.b, i: run.fmt.i, u: run.fmt.u },
									xml: run.xml,
									rPrXml: run.rPrXml,
								})),
							},
		...
		/**
		 * A body child that is not a plain paragraph. Tables and block content controls
