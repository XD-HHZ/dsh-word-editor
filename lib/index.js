...
/**
 * Package version, read from the manifest beside this file so it cannot drift from
 * `package.json`. The read is lazy and cached: `/ping` is a health probe, so a
 * missing or unreadable manifest must never prevent the plugin from loading.
 * @returns a promise of the version string.
 */
function version() {
...
export function apply(ctx) {
  ...
  register(ROUTE_PREFIX + '/ping', 'exact', async (req, res) => {
    sendJson(res, 200, { ok: true, version: await version() })
  })
