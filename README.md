...
- 打开再保存、什么都没改，`document.xml` **逐字节不变**（真实 Word 文件上验证过）；
...
- 只读到「段落级」：Word 的字符样式（如超链接样式）虽然跟着原 run 保留，但工具栏只有加粗/斜体/下划线；
...
npm run structure     # 真实文档结构的防数据丢失测试（34 项断言）
```

`roundtrip` 覆盖最容易悄悄坏掉的部分：无改动时逐字节不变、改文案时 `pPr` 与未改动 run 原样保留、**只改格式（比如只加粗、不动文字）也必须被检测到**、样式/列表切换时 `pPr` 是定点修补而不是重建、新段落与空段落的输出形状。

`structure` 是防数据丢失的那一道：夹具（`test/fixtures.mjs`，XML 字符串，仓库里不放二进制）覆盖表格、超链接、图片、域、换行、书签与块级内容控制，断言**打开+保存逐字节不变**、非段落子元素不丢、只读段落即使被伪造的 DOM 数据篡改也保持原字节。它特意调用保存按钮真正调用的那个 `saveDocumentXml`，而不是底层的 emitter——早期测试就是这里出了偏差：绕过保存路径去测 emitter，于是「保存时表格被整个丢掉」这个 bug 一直没被发现。
...
# NO-OP INVARIANT OK (byte-for-byte)
```
