...
> **请使用 1.2.0 及以上版本。** 1.0.0 与 1.1.0 有一个会丢内容的缺陷：保存时会把表格、
> 块级内容控制这类非段落结构从文件里删掉（只影响保存，打开不保存不会损坏）。详见
> [CHANGELOG](CHANGELOG.md)。

## 它做什么

- 在右侧栏文件树里点击任意 `.docx`，直接进入可编辑视图（标题、段落、加粗/斜体/下划线、字号、颜色、有序/无序列表）；
- 点「保存」写回**同一个文件**；
...
> 如果你装的是 1.0.0 / 1.1.0：那两个版本保存时会删除表格等非段落内容，请按上面的方式更新到
> 1.2.0 及以上。已经保存过的文档需要从备份或版本控制里恢复——插件不会替你修复已损坏的文件。
